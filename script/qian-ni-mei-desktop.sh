...
# =========================================================
# 签你妹 for desktop
# Auto-check attendance for zmz2017.com
# by: larsenlouis
...
curl -s "http://www.zmz2017.com/user/login/" -c session-$timestamp.txt > /dev/null
...
json=$(curl -s "http://www.zmz2017.com/user/login/ajaxlogin" -b session-$timestamp.txt --data "account=$user&password=$password&remember=0&url_back=" -c usercookie-$timestamp.txt)
...
curl -s "http://www.zmz2017.com/user/sign" -b usercookie-$timestamp.txt > /dev/null
